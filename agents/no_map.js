...
function getRawPrompt() {
  ...
  if (
    ...
    // raw onMap
    prompt += `\nMap width: ${rawOnMap.width}\nMap height: ${rawOnMap.height}\nTiles are arranged as ${rawOnMap.height} rows in ${rawOnMap.width} columns.`; //${JSON.stringify(noParcelSpawnerTiles, null, 3)}\n`;
    if (GOAL == "deliver") {
      prompt += `\nThe available delivery points are: ${noParcelSpawnerTiles
